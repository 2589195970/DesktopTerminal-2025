...
This is **zdf-exam-desktop**, a Qt5-based secure kiosk browser designed for educational exam environments. The application creates a locked-down web browser that restricts user access to only specified URLs and prevents unauthorized system access through comprehensive shortcut blocking and fullscreen enforcement.

## Architecture

**Main Components:**
- `ShellBrowser` - Main Qt WebEngine-based kiosk application class
- `Logger` - Singleton logging system with category-based log files  
- `ConfigManager` - Singleton configuration management with JSON support
- `GlobalEventFilter` - System-wide event interceptor for security restrictions
- `QHotkey` - Third-party library for global hotkey support (submodule)

**Security Model:**
- Fullscreen kiosk mode with window state enforcement
- Global keyboard shortcut blocking (except Ctrl+R refresh)
- Password-protected exit via F10 or backslash hotkeys
- Comprehensive logging of all user interactions and exit attempts
...
- CMake configuration with Qt5 paths
- Icon conversion (PNG/SVG/ICO → ICNS)
- App bundle creation with proper structure
- Qt framework deployment via macdeployqt
- Code signing and DMG generation

### Manual Build (All Platforms)
```bash
cd zdf-exam-desktop
mkdir build && cd build
cmake .. -DCMAKE_PREFIX_PATH=/path/to/qt5 -DCMAKE_BUILD_TYPE=Release
cmake --build . --config Release
```

### Test Hotkey Component
```bash
cd zdf-exam-desktop/QHotkey/HotkeyTest
mkdir build && cd build
...
The application uses dynamic JSON configuration with the following search priority:
1. Executable directory: `./config.json`
2. User config: `~/.config/zdf-exam-desktop/config.json` (Unix) or `%APPDATA%\zdf-exam-desktop\config.json` (Windows)
3. System config: `/etc/zdf-exam-desktop/config.json` (Unix only)
4. Resources fallback: `resources/config.json`

**Required Configuration Fields:**
- `url` - Target website URL for the kiosk
- `exitPassword` - Password for secure application exit
- `appName` - Application window title

## Dependencies

**Qt5 Components Required:**
- Qt5::Core, Qt5::Widgets
- Qt5::WebEngineWidgets, Qt5::WebEngine

**Platform Dependencies:**
- macOS: Qt5 via homebrew (`brew install qt@5`)
- macOS: jq for JSON parsing (`brew install jq`)
- Optional: ImageMagick for icon conversion
- Optional: Inkscape for SVG icon support

## Development Notes

**Configuration Testing:**
- Modify `resources/config.json` to test different URLs and settings
- Application automatically creates default config if none found
- See `docs/config-guide.md` for detailed configuration documentation

**Security Features:**
- All system shortcuts are blocked via `GlobalEventFilter`
- Only Ctrl+R (refresh) is permitted for web content
- Exit requires password entry via secure dialog
- Comprehensive logging tracks all security-related events

**Platform-Specific Considerations:**
- macOS: Requires proper code signing for distribution
- macOS: WebEngine frameworks need manual copying in some cases
- Windows: Uses NSIS installer script (`installer.nsi`)
- Cross-platform hotkey support via QHotkey library

## Logging

Application creates separate log files in `./log/` directory:
- App operations and lifecycle events
- Configuration loading and validation
- Hotkey registration and usage
- Startup and shutdown processes

Each log category can be individually controlled and has automatic log rotation.